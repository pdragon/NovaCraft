...
partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        // Progress<double> в Avalonia автоматически захватывает SynchronizationContext.
        // Все вызовы Report() придут в этот (UI) поток без Dispatcher.Invoke.
        Progress = new Progress<TorrentSessionState>(state =>
        {
            // Тут потом добавим развилку на тип качки, а пока у нас только DHT протокол будет толькко он
            _seedButton.Content = state.State == csdl.Enums.TorrentState.Seeding ? "⏹ Остановить раздачу" : "🚀 Начать раздачу";
    ...
    */
    ...
    {
        if (_seedButton.Content?.ToString() == "⏹ Остановить")
        {
            Cts?.Cancel();
            _seedButton.Content = "🚀 Начать раздачу";
            _seedButton.Classes.Remove("active");
            _statusText.Text = "Остановлено";
            return;
        }

        _seedButton.Content = "⏹ Остановить";
        _seedButton.Classes.Add("active");
        _statusText.Text = "Инициализация...";
        ...
        //catch (OperationCanceledException) { StatusText.Text = "Остановлено пользователем"; }
        //catch (Exception ex) { StatusText.Text = $"❌ Ошибка: {ex.Message}"; }
        //finally
        //{
        //    SeedButton.Content = "🚀 Начать раздачу";
