...
namespace Novacraft.Library
{
    ...
    public static class ForgeThingy
    {
        ...
        //public static NovacraftAddonJson GetAddonJson(string version, NovacraftMainJson main, bool online)
        public static NovacraftAddonJson GetAddonJson(LauncherConfig.ModPack selectedModPack, NovacraftMainJson main, bool online)
        {
            string version = selectedModPack.Version.Id;
            //var dir = Path.Combine(Path.GetTempPath(), ".Novacraft-forge");
            ...
            return progress.Start(ctx => {
                ...
                //var jar = Path.Combine(Path.GetTempPath(), ".Novacraft-forge", "installer.jar");
            ...
            return progress.Start(ctx => {
                ...
                NovacraftMainJson data = new();
                bool skipInstallerJson = false;
                switch (ForgeJson.GetType(content))
                {
                    case ForgeJson.Type.PreLegacy:
                        data = ForgeLegacyInstallerJson.ForgeToNovacraftPartial(JsonConvert.DeserializeObject<ForgeLegacyInstallerJson>(content));
                        skipInstallerJson = true;
                        break;
                    case ForgeJson.Type.Legacy:
                        data = NovacraftMainJson.MojangToNovacraftPartial(JsonConvert.DeserializeObject<MojangLegacyMainJson>(content));
                        break;
                        case ForgeJson.Type.Ordinary:
                        data = NovacraftMainJson.MojangToNovacraftPartial(JsonConvert.DeserializeObject<MojangMainJson>(content));
                        break;
                }

                // var data = MojangLegacyMainJson.IsLegacyJson(content) 
                //     ? NovacraftMainJson.MojangToNovacraftPartial(JsonConvert.DeserializeObject<MojangLegacyMainJson>(content)) 
                //     : NovacraftMainJson.MojangToNovacraftPartial(JsonConvert.DeserializeObject<MojangMainJson>(content));                
                task.Description = "Processing addon";
                var addon = new NovacraftAddonJson {
                ...
                if (!skipInstallerJson)
                {
                    ...
                    var dataInst = NovacraftMainJson.MojangToNovacraftPartial(dataInstaller);
        ...
        //public static string GetForgeData(string version, NovacraftMainJson main, bool online)
        public static string GetForgeData(LauncherConfig.ModPack selectedModPack, NovacraftMainJson main, bool online)
        {
            ...
            return progress.Start(ctx =>
            {
                ...
                //var dir = Path.Combine(selectedModPack.PackPath, ".Novacraft-forge");
        ...
        /// <returns>The addon JSON</returns>
        public static void RunProcessors(LauncherConfig.ModPack modpack, NovacraftMainJson main, bool online)
        {
            var progress = AnsiConsole.Progress()
                .HideCompleted(true)
                .Columns(new TaskDescriptionColumn(),
            ...
            progress.Start(ctx => {
                ...
                foreach (var lib in dataInstaller.Libraries) {
                    ...
                    FilesManager.DownloadLibrary(new NovacraftMainJson.JsonLibrary {
                ...
                for (var index = 0; index < dataInstaller.Processors.Length; index++) {
                    ...
                    var file = FilesManager.GetLibraryPath(modpack, new NovacraftMainJson.JsonLibrary { Path = dataInstaller.Libraries
                    ...
                    foreach (var str in proc.Classpath) {
                        var file2 = FilesManager.GetLibraryPath(modpack, new NovacraftMainJson.JsonLibrary { Path = dataInstaller.Libraries
        ...
        //public static void Run(NovacraftMainJson main, NovacraftAddonJson addonMain, Account account, string maxRam, bool customWindowSize, float width, float height, bool online, string gamePath)

        /*
        public static void _Run(NovacraftMainJson main, NovacraftAddonJson addonMain, Account account, bool online, LauncherConfig.ModPack modpack)
        {
            ...
            foreach (var library in main.Libraries)
            {
                ...
                {
                    var file2 = FilesManager.GetLibraryPath(modpack, new NovacraftMainJson.JsonLibrary
                    {
                        Path = library.Path
                    });
        ...
        */
        /*
        public static void RunWithoutProcessors(NovacraftMainJson main, ForgeInstallerJson dataInstaller, bool online)
        {
            var file = FilesManager.GetLibraryPath(new NovacraftMainJson.JsonLibrary
            {
            ...
            foreach (var str in proc.Classpath)
            {
                var file2 = FilesManager.GetLibraryPath(new NovacraftMainJson.JsonLibrary
                {
                    Path = dataInstaller.Libraries
                    .FirstOrDefault(x => x.Name == str)!.Downloads.Artifact.Path
                });
            ...
            foreach (var arg in proc.Arguments)
            {
                ...
                if (replaced.StartsWith("/") || replaced.StartsWith("\\")) replaced =
                    Path.Combine(Path.GetTempPath(), ".Novacraft-forge", replaced
                        .Substring(1, replaced.Length - 1)
                        .Replace('/', '\\'));
                args.Append($"{replaced} ");
            }
        ...
        */
        ...
        {
            // TODO: read .Novacraft\forge\install-1.19.2.json and .Novacraft\forge\version-1.19.2.json and check for exist all libraries
